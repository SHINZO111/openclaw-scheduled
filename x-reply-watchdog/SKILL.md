...
- 失敗・エラー・タスク未登録・スクリプトクラッシュ等、**いかなる状況でもcronツールを使ってはならない**
- 自己再起動・別ジョブ起動・スケジュール変更も**完全禁止**
...
⚠️ **スケジュール実行の必須ルール**: どのステップで失敗・中断しても、必ず何らかのメッセージを返して終了すること。cronツールは使用禁止——エラーはDiscordで報告して終了。
