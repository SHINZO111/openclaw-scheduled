...
description: 毎日11:00 JST - AIエージェントシステムをXに投稿 @KURAOpenclaw
---
...
## タスク: AIエージェントシステム最新情報を@KURAOpenclawにポスト
...
- **冒頭フック（30文字以上）**:
  ...
  ※このアカウントは特に「正直な告白トーン」を強めに出す。「実は自分も半信半疑だった」「正直ここまで来るとは思ってなかった」のように、書き手の本音や過去の判断ミスを隠さない姿勢が差別化ポイント。
...
## 今週のトレンド指示（x-trend-injection-weekly 2026-05-25）
- 今週の注目トレンド1: エンタープライズAI導入加速 → 積極的に言及する
- 今週の注目トレンド2: AIエージェントと自動化 → 関連づけて投稿
- 今週の注目トレンド3: ローカルAIとエッジコンピューティング → 必要に応じて言及

## Step 1: 最新ニュース収集 & バズネタ選定
利用可能なweb検索ツール（`web_search`、`brave_web_search`、`gog` 等、いずれか使えるもの）を使用して、過去48時間以内のAIエージェント関連ニュースを検索する。
検索キーワード例: "AI agent 2026", "autonomous AI agent", "multi-agent system", "AIエージェント 最新", "agentic AI", "AI agent autonomous 2026", "AIエージェント 自律 最新", "agentic AI 衝撃", "AIエージェント 仕事 変える"
...
> **[採点調整] エージェント系ニュースは意外性スコアを+1で評価可**（AIエージェントが既存の仕事・ワークフローを変えるという内容は、読者に与えるインパクトが大きいため）。
...
**メイン投稿（250〜400文字・ハッシュタグなし）:**
フックで始め、「詳細はリプ欄へ」で締める（URLはリプライ末尾に自動付与）。
...
> ### ⚠️ リプライ未投稿リカバリー（exitCode=1 かつ "投稿後のURL取得に失敗" の場合）
> メインツイートは投稿済みだがリプライが投稿されていない場合がある。
> その場合、実際に投稿されたツイートURLを確認し（プロフィールページ等）、以下のコマンドで **reply-only.js** を使ってリプライのみ再投稿すること:
>
> ```powershell
> # reply-data.json を作成
> $replyData = @{
...
>     replyUrl = "[参照元URL]"
> }
> $replyData | ConvertTo-Json -Depth 3 | Out-File "$env:TEMP\reply-data.json" -Encoding UTF8
>
> # タスクスケジューラ経由で実行（Playwright がブロッキングするため）
> $taskName = "XReplyNow_$(Get-Date -Format 'HHmmss')"
> $logFile  = "$env:TEMP\reply-task.log"
> $batFile  = "$env:TEMP\run-reply.bat"
> "@echo off`r`ncd /d `"C:\Users\sawas\.openclaw\workspace\tools\x-poster`"`r`n`"C:\Program Files\nodejs\node.exe`" reply-only.js `"[ツイートURL]`" `"$env:TEMP\reply-data.json`" > `"$logFile`" 2>&1" | Out-File $batFile -Encoding ASCII
...
> # 60秒後に確認
> Start-Sleep 60; Get-Content $logFile
> ```
>
> **reply-only.js の注意点**:
> - `focusAndPaste` はクリップボードペースト方式（`Ctrl+V`）を使用。`keyboard.type` では React テキストエリアに入力できない
> - 投稿後に「Try Boosting this post!」モーダルが出る場合があるが、自動で閉じる処理あり
> - スクリプトパス: `C:\Users\sawas\.openclaw\workspace\tools\x-poster\reply-only.js`
Step 2で厳選した記事の**参照元URL**を含めて投稿する（TwitterがOG画像カードを自動表示）。
...
$sourceUrl = "[Step 2で収集した記事の参照元URL]"

# og:image 取得（ネイティブ画像添付でアルゴリズム有利・OGカードより高リーチ）
$tmpImage = $null; $ogUrl = $null
try {
    $html = (Invoke-WebRequest -Uri $sourceUrl -UseBasicParsing -TimeoutSec 10 -ErrorAction Stop).Content
    if ($html -match 'property=["']og:image["'][^>]*content=["']([^"']+)["']') { $ogUrl = $Matches[1] }
    elseif ($html -match 'content=["']([^"']+)["'][^>]*property=["']og:image["']') { $ogUrl = $Matches[1] }
...
} catch { Write-Host "og:image取得失敗（続行）: $_"; $tmpImage = $null }
# JSON: url は渡さない(メインにURL含めない→アルゴリズム有利) / replyUrlでリプライ末尾に「参考: URL」付与
if (-not $reply.Trim()) {
...
if ($exitCode -eq 0) {
    ...
    $logger = "C:\Users\sawas\.openclaw\workspace\tools\x-post-logger.py"
    $logScript = "$python $logger -skill 'x-post-ai-agent' -content '$($main.Trim())' -success true"
    Invoke-Expression $logScript
    ...
    $python = "C:\Program Files\Python312\python.exe"
    $logger = "C:\Users\sawas\.openclaw\workspace\tools\x-post-logger.py"
    $logScript = "$python $logger -skill 'x-post-ai-agent' -content '$($main.Trim())' -success false"
    Invoke-Expression $logScript
    exit $exitCode
}
```

投稿成功後にTWEET_URLを確認してください。失敗した場合はエラー内容を報告してください。
...
- 各テーマごとに過去のエンゲージメントデータに基づく高エンゲージメント時間帯に設定して発火させる。
- 任意の2つのX投稿（他のテーマを含む）の間隔は最低20分以上あけること。これによりクラスタリングやレートリミットを防止できる。
- 現在のデフォルトスケジュールは各スキルの説明に記載の時刻（HH:MM JST）だが、必要に応じて調整してください。
