---
name: x-post-ai-finance
description: |
  OpenClaw から X（Twitter、@KURAOpenclaw アカウント）へ AI金融 / フィンテック関連の自動投稿を行うスキル。
  Playwright＋Cookie認証ベースで、投稿プロトコル（本文180-270字・リプ600-1000字、URL必須、ハッシュタグ/絵文字禁止、末尾「詳細はリプ欄へ」）を含む。
  以下のフレーズで起動する：
  - 「AI金融をXで投稿して」
  - 「フィンテックをツイートして」
  - 「AI finance投稿」
  `post-to-x.bat` / `post-to-x.js` をPowerShellから呼び出す。AI×金融・フィンテック・決済・ブロックチェーン・AI投資分析等発信用途。
---
...
- `AI 金融 フィンテック 最新`
- `AI fintech banking investment news`
- `生成AI 金融機関 導入 事例`
- `AI trading OR AI underwriting OR AI compliance`

`web_search` が 429（レートリミット）で失敗した場合、以下の直接フェッチに切り替える（優先順）:
1. `web_fetch` で `https://www.finextra.com/` を確認
2. `web_fetch` で `https://techcrunch.com/category/fintech/` を確認
3. `web_fetch` で `https://gigazine.net/` のトップページを確認してAI金融関連記事を探す
4. `web_fetch` で `https://www.bloomberg.co.jp/` を確認（日本語）
...
| x-post-ai-finance | 毎日 21:00 JST | AI金融・フィンテック（本スキル） |
| x-dm-welcome-daily | 毎日 22:00 JST | 新フォロワーDMウェルカム |

**Discord報告時の必須パラメータ**: `message` ツールは `target`（または `channel`）省略時にエラーになる。報告時は必ず以下を明示指定すること:
```
target: "discord:1489796417449889844"
```
（guild: 1489796417449889842 / channel: 一般。openclaw.json の channels.discord.guilds 設定と一致）

**起動メッセージ形式**:
```
【X投稿 @KURAOpenclaw - AI金融 / フィンテック最新情報】
1. web_searchで「AI 金融 フィンテック 最新」「AI fintech」等を検索し重要トピック1件選択。失敗時はweb_fetchで直接ニュースサイトを確認（finextra/techcrunch/gigazine/bloomberg/itmedia）
