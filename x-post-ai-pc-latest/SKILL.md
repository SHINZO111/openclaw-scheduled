---
name: x-post-ai-pc-latest
description: |
  OpenClaw から X（Twitter、@KURAOpenclaw アカウント）へ AI PCハードウェア / 新製品スペック関連の自動投稿を行うスキル。
  以下のフレーズで起動する：
  - 「AI PCハードウェアをXで投稿して」
  - 「新型AI PCをツイートして」
  `post-to-x.bat` / `post-to-x.js` をPowerShellから呼び出す。Qualcomm/Intel/AMD新チップ・Copilot+PC新製品・NPUベンチマーク発信用途。
---
...
3. ファイルが存在しない or 10分超過（stale）→ 続行
...
- 冒頭フック（30文字以上）: 具体的な固有名詞・数字・スペック値を含める
- 段落分割（`\n\n`）で3〜4段落に
...
- `Qualcomm OR Intel OR AMD AI PC 新製品 スペック`
- `Copilot+ PC 新モデル 発売`
- `NPU ベンチマーク AI PC review`

`web_search` が 429（レートリミット）で失敗した場合:
1. `web_fetch` で `https://www.tomshardware.com/laptops` を確認
2. `web_fetch` で `https://www.notebookcheck.net/` を確認
3. `web_fetch` で `https://pc.watch.impress.co.jp/` を確認
4. `web_fetch` で `https://www.theverge.com/tech` を確認
...
| 3 | 認証失敗 | `.env` 更新 → `node setup-cookies.js` |
...
| x-post-ai-pc-latest | 毎日 08:00 JST | AI PCハードウェア・新製品（本スキル） |
...
**起動メッセージ形式**:
```
【X投稿 @KURAOpenclaw - AI PCハードウェア最新情報】
1. web_searchで「Qualcomm/Intel/AMD AI PC 新製品」等を検索。失敗時はweb_fetchでtomshardware/notebookcheck/pc.watch/thevergeを確認
...
6. 結果をDiscordに報告
7. 【Threadsクロスポスト】X成功後: post-to-threads.bat "threads_main（500字以内）" "threads_reply（500字以内）"
```
