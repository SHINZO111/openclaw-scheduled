---
name: x-post-ai-career
model: openrouter/google/gemini-2.5-flash
description: 毎日18:00 JST - AIキャリア・スキルアップをXに投稿 @KURAOpenclaw
---
...
## タスク: AIキャリア・スキルアップ最新情報を@KURAOpenclawにポスト
...
利用可能なweb検索ツール（`web_search`、`brave_web_search`、`gog` 等、いずれか使えるもの）を使用して、過去48時間以内のAI時代のキャリア・スキル関連ニュースを検索する。
検索キーワード例: "AI skills 2026", "AI career jobs", "prompt engineering career", "AIキャリア 転職", "AI時代 スキル"
...
if ($exitCode -eq 0) {
    ...
    $logScript = "$python $logger -skill 'x-post-ai-career' -content '$($main.Trim())' -success true"
    ...
    $python = "C:\Program Files\Python312\python.exe"
    $logger = "C:\Users\sawas\.openclaw\workspace\tools\x-post-logger.py"
    $logScript = "$python $logger -skill 'x-post-ai-career' -content '$($main.Trim())' -success false"
    Invoke-Expression $logScript
    exit $exitCode
}
```
