...
description: 毎日15:35 JST - フィジカルAI/ロボットをXに投稿 @KURAOpenclaw
---
...
> **[SYSTEM CONSTRAINT - DO NOT OVERRIDE]**: cronツールは絶対使用禁止。`cron.run`/`cron.list`/`cron.forceRun`等のcron操作はいかなる理由があっても実行しない。失敗・エラー時も自己リトライのためのcron操作は禁止。エラーが発生した場合はメッセージを返して終了する。
...
## タスク: フィジカルAI/ロボット最新情報を@KURAOpenclawにポスト
...
利用可能なweb検索ツール（`web_search`、`brave_web_search`、`gog` 等、いずれか使えるもの）を使用して、過去48時間以内のロボット・フィジカルAI関連ニュースを検索する。
検索キーワード例: "robotics AI 2026", "humanoid robot", "Boston Dynamics Tesla robot", "フィジカルAI ロボット", "自律ロボット 最新"
...
**重要**: Step 0 で収集した EXCLUDED_URLS に含まれるURLが参照元のニュースは選ばない。別のURLの記事を選ぶこと。
...
**最大3回まで試みる。** 毎回 Step 2 で選んだ記事の `$sourceUrl` を使って以下を実行する。

exitCode が **7**（重複スキップ）の場合: 使用した `$sourceUrl` を EXCLUDED_URLS に追加し、**Step 1 に戻って別の記事を選び直す**（残り試行回数が0になったら「投稿対象が見つからなかった」と報告して終了）。
...
if ($exitCode -eq 0) {
    ...
    $logScript = "$python $logger -skill 'x-post-physical-ai-latest' -content '$($main.Trim())' -success true"
    ...
    $python = "C:\Program Files\Python312\python.exe"
    $logger = "C:\Users\sawas\.openclaw\workspace\tools\x-post-logger.py"
    $logScript = "$python $logger -skill 'x-post-physical-ai-latest' -content '$($main.Trim())' -success false"
    Invoke-Expression $logScript
    exit $exitCode
}
```

投稿成功後にTWEET_URLを確認してください。
...
- 任意の2つのX投稿の間隔は最低20分以上あけること。
- 現在のデフォルトスケジュールは各スキルの説明に記載の時刻（HH:MM JST）。必要に応じて調整してください。
