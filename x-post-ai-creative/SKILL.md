...
description: |
  OpenClaw から X（Twitter、@KURAOpenclaw アカウント）へ AIクリエイティブ（画像生成・動画生成・音楽生成AI）関連の自動投稿を行うスキル。
  Playwright＋Cookie認証ベースで、投稿プロトコル（本文180-270字・リプ600-1000字、URL必須、ハッシュタグ/絵文字禁止、末尾「詳細はリプ欄へ」）を含む。
  以下のフレーズで起動する：
  - 「AIクリエイティブをXで投稿して」
  - 「画像生成AIをツイートして」
  - 「AIクリエイティブ投稿」
  `post-to-x.bat` / `post-to-x.js` をPowerShellから呼び出す。Midjourney/Sora/Runway/Suno等の画像・動画・音楽生成AI発信用途。
---
...
- `Midjourney OR Sora OR Runway 新機能 最新`
- `AI image generation OR video generation OR music generation news`
- `生成AI クリエイティブツール アップデート`
...
2. `web_fetch` で `https://www.theverge.com/ai-artificial-intelligence` を確認
3. `web_fetch` で `https://gigazine.net/` のトップページを確認してAIクリエイティブ関連記事を探す
4. `web_fetch` で `https://www.itmedia.co.jp/aiplus/` を確認（日本語記事）
5. `web_fetch` で `https://huggingface.co/papers?q=image+generation` を確認（HuggingFace 最新AI論文・実装紹介）
...
| x-post-ai-creative | 毎日 10:00 JST | AIクリエイティブ / 画像・動画・音楽生成（本スキル） |
| x-post-ai-agent | 毎日 12:00 JST | AIエージェント / マルチエージェント |
| x-post-ai-security | 毎日 12:50 JST | AIセキュリティ |
| x-post-physical-ai-latest | 毎日 18:00 JST | フィジカルAI / ロボット |

**起動メッセージ形式**:
```
【X投稿 @KURAOpenclaw - AIクリエイティブ最新情報】
1. web_searchで「Midjourney/Sora/Runway 新機能」「AI image/video/music generation」等を検索し重要トピック1件選択。失敗時はweb_fetchで直接ニュースサイトを確認（techcrunch/theverge/gigazine/itmedia/huggingface）
