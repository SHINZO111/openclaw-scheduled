---
name: x-post-ai-creative
description: 毎日10:00 JST - AIクリエイティブツールをXに投稿 @KURAOpenclaw
---
...
## タスク: AIクリエイティブツール最新情報を@KURAOpenclawにポスト
...
利用可能なweb検索ツール（`web_search`、`brave_web_search`、`gog` 等、いずれか使えるもの）を使用して、過去48時間以内のAIクリエイティブツール関連ニュースを検索する。
検索キーワード例: "AI image generation 2026", "Midjourney Sora video AI", "AI art tools", "AI動画生成 最新", "AIクリエイティブ"
...
if ($exitCode -eq 0) {
    ...
    $logScript = "$python $logger -skill 'x-post-ai-creative' -content '$($main.Trim())' -success true"
    ...
    $python = "C:\Program Files\Python312\python.exe"
    $logger = "C:\Users\sawas\.openclaw\workspace\tools\x-post-logger.py"
    $logScript = "$python $logger -skill 'x-post-ai-creative' -content '$($main.Trim())' -success false"
    Invoke-Expression $logScript
    exit $exitCode
}
```
